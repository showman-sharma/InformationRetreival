...
class InformationRetrieval():
	...
	def buildIndex(self, docs, docIDs, docTitles):
		...
		t1 = time.perf_counter()
		vocab = list(self.index.keys())
		self.NESAmatrix = self.NESA(docTitles,vocab)
		self.docVecs = {docID:np.dot(self.docVecs[docID],self.NESAmatrix) for docID in self.docVecs}
		t2 = time.perf_counter()
		print('Time taken to form NESA matrix= {} minutes'.format((t2-t1)/60))
	...
	def NESA(self,docTitles,vocab):
		try:
			with open('NESAmatrix.npy','rb') as f:
				NESAmatrix = np.load(f)
				return NESAmatrix
		except:
			pass
		try:
					...
					for word in sentence:
						if word not in stop_words:
							try:
								...
								index[word] = [i]
							proc.append(word)
					procSummary.append(proc)
					concepts.append(concept)
			N = len(procSummary)
		except:
			esa = self.ESA(docTitles,vocab)
			return self.NESA(docTitles,vocab)
		new_index = {}
		for t in index:
			c = Counter(index[t])
			if len(c)<0.1*N and len(index[t])>1:
				new_index[t] = c	
		del index 
		index = new_index		
		t2 = time.perf_counter()
		print('Time taken to form NESA index = {} minutes'.format((t2-t1)/60))
		vocab_len = len(index)


		#Building tf matrix
		t1 = time.perf_counter()
		tf_matrix = np.array([np.array([0]*vocab_len if len(procSummary[i])==0 else [index[t][i]/len(procSummary[i]) for t in index])	for i in range(N)])	
		t2 = time.perf_counter()
		print('Time taken to form NESA tf vectors= {} minutes'.format((t2-t1)/60))
		print('TF matrix shape:',tf_matrix.shape)
		for j in range(N):
			tf_matrix[:,j] = tf_matrix[:,j]/max(np.linalg.norm(tf_matrix[:,j]),1e-6)

		NESA_matrix = tf_matrix@tf_matrix.T
		with open('NESAmatrix.npy','wb') as f:
			np.save(f, NESA_matrix)
